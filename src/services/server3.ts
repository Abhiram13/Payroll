...
class MyResponse<Request extends http.IncomingMessage = http.IncomingMessage> extends http.ServerResponse<Request> {
   ...
   json(body: any): void {
      ...
      try {
         ...
         this.write(`{ "statusCode": ${StatusCode.SERVER_ERROR}, "message": "Some internal error at parsing given response" }`);
         this.end();
      }    
   }
}
   ...
   listen(port: number = 3000, callback: () => void) {
      ...
      this.#httpServer.on('request', (req: Request, res: Response) => {
         ...
         if (!api) {
            ...
            response?.write(JSON.stringify({ statusCode: 404, message: "Route not found/ does not exist!" }));
            response?.end();
            return;
